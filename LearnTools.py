...
def return_args(parser):
    ...
    parser.add_argument("--separator", default=False, action="store_true",
                        help="Different voxel-wise loss for separator material.")
    ...
    parser.add_argument('-g_image_path', type=str, help="Path to the LR 3D volume")
    parser.add_argument('-d_image_path', nargs='+', type=str, help="Path to "
                        "the HR 2D slice, if Isotropic, 3 paths are needed in correct order")
...
class DownSample(nn.Module):
    ...
    reasons.
    """
    def __init__(self, n_dims, low_res_idx, scale_factor,
                 device, super_sampling=False):
        """
    ...
    def voxel_wise_distance(self, generated_im, low_res):
        ...
        device1 = torch.device("cuda:0")                                     #### we introduced it because of the DDP
        low_res = low_res.to(device1)                                        #### we introduced it because of the DDP
    ...
    def forward(self, generated_im, low_res_input=False):
        ...
        device1 = torch.device("cuda:0")                                      #### we introduced it because of the DDP
        generated_im = generated_im.to(device1)                               #### we introduced it because of the DDP
