...
HIGH_L_3D = 256  # length of high resolution 3d
...
class BatchMaker:
    ...
    def __init__(self, device, to_low_idx=None, path=NMC_PATH, sf=4, dims=3,
                 stack=True, down_sample=False, low_res=False, rot_and_mir=True, super_sample=False):
        """
        ...
        self.down_sample, self.to_low_idx = down_sample, to_low_idx
        ...
        if self.down_sample:
            self.down_sample_object = LearnTools.\
                DownSample(self.dims, self.to_low_idx, self.scale_factor, device, super_sample).to(self.device)
            self.im = np.array(self.down_sample_im(self.im).detach().cpu())
            self.phases = [self.phases[0]] + list(np.array(self.phases)[np.array(self.to_low_idx.detach().cpu())])
            self.high_l = int(HIGH_L_3D / self.scale_factor)
    ...
    def rotate_and_mirror(self):
        ...
        for k in np.arange(4):  # for each 90 deg rotation
            ...
            res[second_i:second_i + num_ims, ...] = np.rot90(flip_im, k, [-2, -1])
        self.im = res
    ...
    def random_batch3d(self, batch_size, dim_chosen):
        ...
        """
        res = np.zeros((batch_size, len(self.phases), *self.high_l * np.ones(self.dims, dtype=int)), dtype=self.im.dtype)
    ...
    def generate_a_random_image3d(self, dim_chosen):
        ...
        res_image = self.im[:, s_ind[0]:e_ind[0], s_ind[1]:e_ind[1], s_ind[2]:e_ind[2]]
